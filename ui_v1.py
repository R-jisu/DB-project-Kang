...
class WindowClass(QMainWindow, form_mainclass):
    def __init__(self):
        ...
        self.search.clicked.connect(self.openCaptureClass)
...
class searchwindow(QDialog, QWidget, form_searchclass):
    def __init__(self, foodname):
        ...
        self.S_BackBtn.clicked.connect(self.openCaptureClass)
        self.S_searchBtn.clicked.connect(self.searching)
        self.S_searchBtn.clicked.connect(self.searching)
        self.S_tableWidget.doubleClicked.connect(self.openCaptureClass1)
    ...
    def goRecipeWindow(self):
        self.hide()  # 윈도우 숨김
        recipe = recipewindow()
        recipe.exec()  # recipe 창 닫을 때까지 기다림
        print(2)
        self.show()  # recipe 창을 닫으면 다시 첫 번째 창이 보여짐
...
class recipewindow(QDialog, QWidget, form_recipeclass):
    def __init__(self):
        ...
        self.R_BackBtn.clicked.connect(self.openCaptureClass)
